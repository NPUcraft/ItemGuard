...
public final class FixtureService implements Listener {
    ...
    static final int FURNACE_X = 26;
    static final int STONECUTTER_X = 28;
    static final int VILLAGER_Z = 14;
    ...
    Location furnaceLocation() {
        World world = overworld();
        return new Location(world, FURNACE_X, PLATFORM_Y + 1, SPAWN_Z);
    }

    Location stonecutterLocation() {
        World world = overworld();
        return new Location(world, STONECUTTER_X, PLATFORM_Y + 1, SPAWN_Z);
    }

    Location villagerLocation() {
        World world = overworld();
        return new Location(world, SPAWN_X + 0.5, PLATFORM_Y + 1, VILLAGER_Z + 0.5);
    }
    ...
    Block furnaceBlock() {
        World world = overworld();
        Block block = world.getBlockAt(FURNACE_X, PLATFORM_Y + 1, SPAWN_Z);
    ...
    Block stonecutterBlock() {
        World world = overworld();
        Block block = world.getBlockAt(STONECUTTER_X, PLATFORM_Y + 1, SPAWN_Z);
    ...
    void resetStonecutter() {
        stonecutterBlock();
    }

    void removeHarnessVillagers() {
        World world = overworld();
        for (Entity entity : world.getNearbyEntities(spawn(world), 48, 16, 48)) {
            if (entity instanceof org.bukkit.entity.Villager villager && villager.getScoreboardTags().contains("itemguard-harness")) {
                villager.remove();
            }
        }
    }
    ...
    private void buildPlatform(World world) {
            ...
            for (int z = 0; z <= 32; z++) {
                ...
                for (int y = PLATFORM_Y + 1; y <= PLATFORM_Y + 4; y++) {
                    ...
                    if (!air.getType().isAir()
                            ...
                            && air.getType() != Material.FURNACE
                            && air.getType() != Material.STONECUTTER) {
        ...
        world.getBlockAt(FURNACE_X, PLATFORM_Y + 1, SPAWN_Z).setType(Material.FURNACE, false);
        world.getBlockAt(STONECUTTER_X, PLATFORM_Y + 1, SPAWN_Z).setType(Material.STONECUTTER, false);
        for (int x = 0; x <= 32; x++) {
            for (int z = 0; z <= 32; z++) {
