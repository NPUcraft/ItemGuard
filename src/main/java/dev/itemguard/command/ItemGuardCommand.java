...
public final class ItemGuardCommand implements CommandExecutor, TabCompleter {
    ...
    private Component sourceLine(ItemFlowEvent event) {
        if (event.source() == dev.itemguard.flow.model.FlowSource.HUSKSYNC_DATA_APPLY) {
        ...
        if (event.source() == dev.itemguard.flow.model.FlowSource.CREATIVE_INVENTORY) {
        ...
        if (event.source() == dev.itemguard.flow.model.FlowSource.SHULKER_BOX) {
    ...
    private static String formatCredit(ExpectedFlowCredit credit, Instant now) {
        String hint = credit.oneShot() ? "HINT" : "numeric";
