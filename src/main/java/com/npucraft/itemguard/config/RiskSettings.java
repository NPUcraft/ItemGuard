...
public record RiskSettings(
        ...
        IncidentTtlSettings incidents,
        UnknownGainSettings unknownGain,
        DetectorGateSettings detectorGates
) {
    ...
    public static RiskSettings defaults() {
        ...
        scores.put(SignalType.SHULKER_RAPID_TRANSFER, 0);
        scores.put(SignalType.ILLEGAL_ENCHANTMENT, 40);
        scores.put(SignalType.OVER_LEVEL_ENCHANTMENT, 40);
        scores.put(SignalType.CONFLICTING_ENCHANTMENTS, 10);
        ...
        scores.put(SignalType.COMPONENT_MODIFIED, 0);
        scores.put(SignalType.CUSTOM_ITEM_METADATA, 0);
        scores.put(SignalType.ABNORMAL_DURABILITY, 25);
        scores.put(SignalType.SUSPICIOUS_CONTAINER_CONTENTS, 18);
        scores.put(SignalType.CUSTOM_MAX_STACK, 0);
        ...
        return new RiskSettings(
                ...
                true,
                10,
                IncidentTtlSettings.defaults(),
                UnknownGainSettings.defaults(),
                DetectorGateSettings.defaults()
        );
    }
}
