...
public final class JsonlSerializer {
    ...
    public static String toJson(ForensicLogRecord record, java.time.ZoneId zone) {
        ...
        first = string(json, first, "incidentId", stringOf(record.incidentId()));
        first = string(json, first, "incidentType", record.incidentType());
        first = number(json, first, "incidentRisk", record.incidentRisk());
        first = string(json, first, "incidentAlertTransition", record.incidentAlertTransition());
